...
function validate(
  id: string,
  value: string,
  ctx: { dialCountry?: string; currency?: string; password?: string } = {}
): VResult {
  switch (id) {
    case "name":
    ...
    case "password": {
      if (!value) return { warning: "Password is required" };
    ...
    case "confirmPassword": {
      if (!value) return { warning: "Please confirm your password" };
    ...
    case "investorType":
      if (!value) return { warning: "Please select your investor type to continue" };
    ...
    case "websiteUrl":
      ...
      if (!value.trim().startsWith("https://")) return { error: "Must start with https:// for security" };
    ...
    case "investmentThesis":
      ...
      if (value.trim().length < 30) return { warning: "A bit more detail helps founders understand your focus" };
      if (value.length > INVESTOR_CHAR_LIMITS.investmentThesis) return { error: `Max ${INVESTOR_CHAR_LIMITS.investmentThesis} characters` };
      ...
      {options.map(o => (
        <label
          ...
          className={`flex items-center gap-2 px-3 py-2.5 border rounded-lg cursor-pointer transition-all ${
            selected.includes(o.value) ? "bg-forest text-white border-forest shadow-sm" : "bg-beige/50 border-forest/10 hover:bg-beige"
          }`}
        >
...
// ─── FIXED: Global Currency Dropdown (single, used once above ticket inputs) ──

function TicketCurrencyDropdown({
  value,
  onChange,
}: {
  value: string;
        ...
        <div className="absolute right-0 top-full mt-1.5 w-64 bg-white border border-forest/12 rounded-xl shadow-2xl z-[100] overflow-hidden">
          <div className="flex items-center gap-2 px-3 py-2.5 border-b border-forest/8 bg-forest/2">
            ...
            {search && (
              <button
                type="button"
                onClick={() => setSearch("")}
                className="text-forest/30 hover:text-forest/60 transition-colors"
              >
                <X className="w-3 h-3" />
              </button>
            )}
          ...
          <ul className="overflow-y-auto max-h-56">
            {filtered.map(c => (
              ...
              >
                <span
                  className={`w-8 font-bold text-[15px] ${value === c.code ? "text-white" : "text-forest/60"}`}
                >
                  {c.symbol}
                </span>
                <span className="flex-1 truncate">{c.name}</span>
                <span
                  className={`text-xs font-medium ${value === c.code ? "text-white/65" : "text-forest/40"}`}
                >
                  {c.code}
                </span>
              </li>
...
// ─── FIXED: Ticket Amount Input — no currency selector inside, just symbol display ──
...
function TicketAmountInput({
  ...
  return (
    <div
      className={`flex border rounded-lg transition-colors bg-white ${
        hasError
    ...
    >
      {/* Static currency symbol — no dropdown here */}
...
export default function InvestorApplyPage() {
  ...
  // ── FIXED: handleCurrencyChange resets countryAutoFilled so label updates ──
  function handleCurrencyChange(code: string) {
    if (!VALID_CURRENCY_CODES.has(code)) return;
    setTicketCurrency(code);
    setCountryAutoFilled(false); // ← reset so label shows "Selected currency" not stale auto-fill text
    if (form.ticketSizeMin) {
  ...
  function validateStep(step: number): boolean {
    ...
    if (step === 0) {
      ...
      if (!form.password) {
        applyResult("password", { warning: "Password is required" });
        ok = false;
      } else if (pwErr) {
        applyResult("password", { error: pwErr });
        ok = false;
      } else {
        applyResult("password", { success: "Password looks strong" });
      }

      if (!form.confirmPassword) {
        applyResult("confirmPassword", { warning: "Please confirm your password" });
        ok = false;
  ...
  const handleNext = () => {
    for (const id of (STEP_FIELDS[currentStep] ?? [])) {
      if (currentStep === 0 && (id === "password" || id === "confirmPassword")) continue;
      const v = getStringField(id);
      applyResult(id, validate(id, v, { dialCountry, currency: ticketCurrency }));
    }

    const required = REQUIRED_FIELDS[currentStep] ?? [];
    const emptyFields = required.filter(id => !getStringField(id).trim());
                  ...
                  {currentStep === 0 && (
                    <section className="space-y-5 animate-fade-in">
                      ...
                      <div>
                        ...
                        <input
                          type="text" id="name" autoComplete="name" maxLength={INVESTOR_CHAR_LIMITS.name}
                          className={`input-field ${fieldCls("name")}`}
                          placeholder="Jordan Wei" value={form.name} onChange={handleChange}
                        />
                        <FieldError msg={errs.name} />
                        <FieldWarn  msg={warns.name} />
                        <FieldOk   msg={oks.name} />
                      ...
                      <div>
                        ...
                        <input
                          ...
                          placeholder="jordan@meridianvc.com" value={form.email} onChange={handleChange}
                        />
                      ...
                      <div>
                        ...
                        <FieldWarn  msg={warns.password} />
                        <FieldOk   msg={oks.password} />
                      ...
                      <div>
                        ...
                        <FieldWarn  msg={warns.confirmPassword} />
                        <FieldOk   msg={oks.confirmPassword} />
                      </div>
                      ...
                      <div>
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          ...
                          <input
                            ...
                            className={`input-field ${fieldCls("mobile")} ${!dialCountry ? "opacity-40 cursor-not-allowed bg-forest/5" : ""}`}
                            placeholder={
                    ...
                    <section className="space-y-5 animate-fade-in">
                      ...
                      <div>
                        ...
                        <InvestorTypeDropdown
                          ...
                          onChange={v => {
                            setForm(prev => ({ ...prev, investorType: v }));
                            applyResult("investorType", validate("investorType", v));
                            setStepBlocked(null);
                          }}
                          hasError={!!errs.investorType}
                          hasWarning={!!warns.investorType}
                        />
                        <FieldError msg={errs.investorType} />
                        <FieldWarn  msg={warns.investorType} />
                      </div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                          ...
                          <input
                            type="text" id="firmName" maxLength={INVESTOR_CHAR_LIMITS.firmName}
                            className={`input-field ${fieldCls("firmName")}`}
                            placeholder="Meridian Ventures" value={form.firmName} onChange={handleChange}
                          />
                          <FieldWarn msg={warns.firmName} />
                          <FieldOk   msg={oks.firmName} />
                        </div>
                        <div>
                          <div className="flex items-center justify-between mb-1">
                          ...
                          <input
                            type="text" id="designation" maxLength={INVESTOR_CHAR_LIMITS.designation}
                            className={`input-field ${fieldCls("designation")}`}
                            placeholder="Partner, Managing Director…" value={form.designation} onChange={handleChange}
                          />
                      ...
                      <div>
                        ...
                        <textarea
                          id="bio" rows={3} maxLength={INVESTOR_CHAR_LIMITS.bio}
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                          ...
                          <input
                            type="text" id="country" maxLength={INVESTOR_CHAR_LIMITS.country}
                        ...
                        <div>
                          ...
                          <input
                            type="text" id="city" maxLength={INVESTOR_CHAR_LIMITS.city}
                            className="input-field"
                            placeholder="San Francisco" value={form.city}
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                          ...
                          <input
                            type="url" id="websiteUrl" maxLength={INVESTOR_CHAR_LIMITS.websiteUrl}
                            className={`input-field ${fieldCls("websiteUrl")}`}
                            placeholder="https://meridianvc.com" value={form.websiteUrl} onChange={handleChange}
                          />
                        ...
                        <div>
                          ...
                          <input
                            type="url" id="linkedinUrl" maxLength={INVESTOR_CHAR_LIMITS.linkedinUrl}
                            className={`input-field ${fieldCls("linkedinUrl")}`}
                            placeholder="https://linkedin.com/in/jordan-wei" value={form.linkedinUrl} onChange={handleChange}
                          />
                          ...
                          {form.linkedinUrl && oks.linkedinUrl && (
                            <a
                              href={form.linkedinUrl} target="_blank" rel="noopener noreferrer"
                    ...
                    <section className="space-y-6 animate-fade-in">
                      ...
                      <div>
                        ...
                        <MultiToggle
                          ...
                          onChange={v => setForm(prev => ({ ...prev, preferredSectors: v }))}
                        />
                      </div>
                      ...
                      <div>
                        ...
                        <MultiToggle
                          ...
                          onChange={v => setForm(prev => ({ ...prev, preferredStages: v }))}
                        />
                      </div>
                        ...
                        <div className="flex flex-wrap gap-2 mt-2">
                          {geoOptions.map(g => (
                            <label
                              key={g}
                              className={`flex items-center gap-2 px-3 py-2.5 border rounded-lg cursor-pointer transition-all ${
                                form.preferredGeographies.includes(g) ? "bg-forest text-white border-forest shadow-sm" : "bg-beige/50 border-forest/10 hover:bg-beige"
                              }`}
                            >
                              <input
                                type="checkbox" className="sr-only"
                                checked={form.preferredGeographies.includes(g)}
                                onChange={() => {
                      ...
                      <div>
                        ...
                        <textarea
                          id="investmentThesis" rows={4} maxLength={INVESTOR_CHAR_LIMITS.investmentThesis}
                        ...
                        />
                        <FieldWarn msg={warns.investmentThesis} />
                        <FieldOk   msg={oks.investmentThesis} />
                      ...
                      <div className="flex items-center gap-3 p-4 bg-beige/50 rounded-xl border border-forest/8">
                        <button
                          type="button" role="switch" aria-checked={form.impactFocused}
                    ...
                    <section className="space-y-5 animate-fade-in">
                      <div>
                        <h2 className="font-serif text-2xl lg:text-3xl text-forest flex items-center">
                          Ticket Size
                          <Tooltip text="Enter your minimum and maximum investment per startup. Use the currency selector to change currency — it applies to both fields. Used for founder matching only, never shown publicly." />
                        </h2>
                        <p className="text-sm text-forest/50 mt-1">Set your capital deployment parameters.</p>
                      </div>

                      {/* ── FIXED: Single currency info bar with one dropdown ── */}
                      <div className="flex items-center gap-3 p-3 bg-forest/5 rounded-xl border border-forest/8">
                        ...
                        <p className="text-[11px] text-forest/60 flex-1">
                          {countryAutoFilled
                            ? <>Currency auto-set from your phone country · </>
                            : <>Selected currency · </>
                          }
                          <span className="font-semibold text-forest">{ticketCurrency} ({currObj.symbol})</span>
                        </p>
                        {/* Single global currency selector — applies to both min & max */}
                        <TicketCurrencyDropdown value={ticketCurrency} onChange={handleCurrencyChange} />
                      </div>

                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                          <label className="label-style mb-1.5 block">Minimum Ticket</label>
                          {/* FIXED: No currency dropdown inside — just amount input */}
                          <TicketAmountInput
                            ...
                            onAmountChange={v => {
                              ...
                              if (!v) {
                                ...
                                if (form.ticketSizeMax && !isNaN(maxNum) && num > maxNum) {
                                  applyResult("ticketSizeMin", { error: "Minimum cannot exceed maximum" });
                                } else {
                                  applyResult("ticketSizeMin", { success: `${currObj.symbol} ${v}` });
                                }
                              }
                            }}
                          ...
                          />
                          <FieldError msg={errs.ticketSizeMin} />
                          <FieldWarn  msg={warns.ticketSizeMin} />
                          <FieldOk   msg={oks.ticketSizeMin} />
                        </div>
                        ...
                        <div>
                          ...
                          <TicketAmountInput
                            ...
                            onAmountChange={v => {
                              ...
                              if (!v) {
                                ...
                                if (form.ticketSizeMin && !isNaN(minNum) && num < minNum) {
                                  applyResult("ticketSizeMax", { error: "Maximum cannot be less than minimum" });
                                } else {
                                  applyResult("ticketSizeMax", { success: `${currObj.symbol} ${v}` });
                                }
                              }
                            }}
                          ...
                          />
                          <FieldError msg={errs.ticketSizeMax} />
                          <FieldWarn  msg={warns.ticketSizeMax} />
                          <FieldOk   msg={oks.ticketSizeMax} />
                        </div>
                      </div>
          ...
          <div className="absolute right-0 top-0 bottom-0 w-72 bg-white shadow-2xl animate-slide-left" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-3 overflow-y-auto">
              {steps.map((step, index) => {
                ...
                return (
                  <button
                    key={step.num} onClick={() => avail && handleStepClick(index)} disabled={!avail}
