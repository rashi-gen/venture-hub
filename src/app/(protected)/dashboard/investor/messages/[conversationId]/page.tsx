...
import { ApiResponse, ConversationWithParty, MessageWithSender } from "@/components/messaging/types";
import { ChatWindow } from "@/components/messaging/ChatWindow";
import { SendMessageForm } from "@/components/messaging/SendMessageForm";

export const metadata = { title: "Chat | Investor Dashboard" };

type PageProps = {
  params: Promise<{ conversationId: string }>;
};

export default async function InvestorChatPage({ params }: PageProps) {
  ...
  // We'll also fetch the conversation to show the other party name in the header
  ...
  try {
    ...
    if (convData.success) {
      conversationMeta = convData.data.find((c) => c.id === conversationId) ?? null;
    }
